...
문서 정보와 본문은 같은 Pager 페이지에서 동일한 `editorSessionKey`를 composition identity로 사용한다. 속성 form registry와
저장 coroutine은 개별 offscreen 페이지보다 길게 유지하되 실제 저장 대상은 현재 `FileKey`로 캡처해, swipe나 rename 중에도
다른 문서의 초안과 저장 결과가 섞이지 않게 한다.
...
이 초기 인덱싱은 frontmatter 필수값 보완 절차다. 화면 조회를 위한 `WorkspaceMetadataIndex`는 활성 작업 공간
단위의 인메모리 스냅샷이며 별도 파일을 만들지 않는다. `FileKey`·파일 정체성·수정 시각과 General `source`,
Project `plot`만 보관하고 Markdown 본문은 보관하지 않는다. `ProjectIndexer`가 이미 읽은 정상 문서의 `plot`을
이 스냅샷에 전달하므로 Project 하이라키가 같은 본문을 다시 읽거나 편집기 cache에 전체 문서를 미리 넣지 않는다.
실제 본문은 파일을 열 때 읽는다.

Project 최초 진입에서는 `ProjectIndexer`가 같은 activation에서 완전히 열거한 폴더·파일 목록과 정규화된
`ProjectConfig`를 immutable 구조 snapshot으로 함께 전달한다. `MainViewModel`은 이를 첫 하이라키 게시에서만
소비해 SAF provider의 동일 목록을 다시 열거하지 않는다. 폴더나 파일 하나라도 열거하지 못했거나 activation
세대가 달라졌으면 구조 snapshot을 제공하지 않고 기존 provider 조회로 되돌아간다. 인덱싱 완료 뒤 앱 내부에서
파일을 생성하거나 생성 재시도를 완료하면 아직 소비하지 않은 구조 snapshot을 무효화한다. 이후 생성·이동·삭제,
외부 변경 검사와 약 15초 주기 검사는 항상 provider를 다시 조회한다.

provider가 0보다 큰 mtime을 제공하면 같은 파일과 같은 mtime에서만 정상 읽기 결과를 재사용한다. mtime이 없거나
0이면 현재 앱 세션의 마지막 정상 메타데이터를 재사용하고, 문서를 명시적으로 다시 선택하거나 앱 내부에서 저장할 때
실제 본문에서 해당 항목을 갱신한다. 따라서 이 provider에서는 앱 밖의 frontmatter 변경이 선택 전 하이라키에 즉시
드러나지 않을 수 있다. 일시적 읽기 실패는 성공 항목으로 취급하지 않아 mtime이 같아도 다음 조회에서 재시도한다.
작업 공간 위치나 Project/General 종류가 바뀌면 이전 스냅샷을 버린다. 활성화와 snapshot 조회를 분리하고 활성화
세대값을 identity에 포함해, A→B→A로 다시 돌아온 경우에도 첫 A의 늦은 스캔이 현재 A 인덱스를 바꾸지 못한다.

General은 파일명 하이라키를 먼저 게시한 뒤 `source`를 최대 4개씩 제한 병렬로 인덱싱한다. 따라서 많은 파일의
frontmatter 처리가 끝나기 전에도 목록과 편집 진입이 가능하다. 기존 source 상태가 있으면 새 파일을 임시 미분류로
즉시 합치고 삭제된 파일을 제거한다. 실행 중인 스캔에 정규 갱신 주기가 오거나 파일 집합이 실제로 달라지면 현재
스캔 직후 한 번 더 실행한다. 먼저 시작한 스캔 결과도 게시 직전 현재 하이라키 파일 집합과 다시 맞춰, 후속 스캔이
끝날 때까지 새 파일의 임시 미분류 표시와 삭제 반영을 유지한다. 일반 1.5초 poll만으로 느린 source 스캔을 연속
재실행하지 않는다.
폴더 선택도 마지막 정상 하이라키를 먼저 적용하고 provider 재조회 결과를 나중에 합쳐 외부 생성·삭제 감지를 유지한다.
Project의 `plot` mtime 조회와 미캐시 파일 읽기도 최대 4개로 제한 병렬 처리하며, 반복 폴링에서는 약 6초 간격으로
전체 메타데이터를 확인한다. 디스크 읽기 중 편집이 시작되거나 예약 저장이 생기면 결과 적용 시 editor cache를 다시
검사해 늦게 도착한 디스크 결과로 새 초안과 해당 `plot` 메타데이터를 덮지 않는다. 초기 Project 인덱싱도 최대 4개
파일을 병렬로 검사하되 본문 대신 `plot`과 읽기 전후가 일치한 mtime만 보관한다. 읽는 동안 mtime이 달라지면 한 번
다시 읽고, 계속 달라지는 파일은 해당 세션 인덱스에서 제외해 오래된 `plot`과 새 mtime을 결합하지 않는다.
...
막으며, 같은 원문일 때만 cache 객체를 재사용한다. 일반 본문·속성 저장의 외부 쓰기 경계는 §10의 외부 우선 정책을 따른다.
...
플랫폼별 기준은 앱 lifecycle의 단순 `resume`이 아니라 **실제 window focus**다.

- Desktop: 최소화·다른 창 활성화 때 비활성화되고, 작업표시줄·Alt-Tab·클릭으로 MaChum 창이 다시 포커스를 얻을 때 활성화된다.
- Android: 다른 앱에서 돌아온 뒤 Activity 창이 실제 포커스를 얻을 때 활성화된다. 분할 화면에서 보이기만 하고 다른 앱이 포커스된 상태는 비활성이다.
- MaChum이 계속 포커스된 채 사용자가 입력하지 않는 상태는 활성 상태를 유지하므로 폴링도 계속된다.

활성 전환 직후에는 전체 폴더·파일 목록을 검사하고, 이후 활성 상태에서는 열린 문서를 1.5초마다 검사하며 전체 하이라키는
10 tick마다 약 15초 간격으로 검사한다. `MainScreen` 최초 구성 시 창이 이미 포커스되어 있으면 초기 `true`도 활성 전환으로
전달된다. Project 인덱스 구조 snapshot을 초기 하이라키에 재사용할 때는 이 초기 신호가 곧바로 같은 목록을 재조회하지 않도록,
같은 작업공간 activation의 정상 snapshot을 한 번 소비했고 activation 뒤 다른 포커스 신호가 없었던 경우에만 최초 전체 검사를
생략한다. 최초 신호가 `false`인 경우와 실제 `false→true` 복귀는 snapshot 준비 순서와 관계없이 즉시 전체 목록을 조회한다.

### 검사 순서

1. 작업 공간의 파일 목록 재조회와 마지막 정상 하이라키 우선 게시
...
7. 전체 하이라키와 Project `plot` mtime 확인은 본문 폴링과 분리해 약 15초 간격으로 실행하고 변경 파일만 재파싱한다.
   General `source` 전체 스캔은 최초 로드·파일 집합 변경·명시 작업에서 실행하며, 선택하거나 외부 변경을 감지한 문서는
   해당 source 항목만 갱신한다.
...
본문 입력을 예약할 때 마지막 디스크 기준 `NoteFile`을 함께 보관한다. debounce 자동 저장은 실제 쓰기 직전에 파일을
다시 읽고 이 기준과 비교한다. 일치할 때만 로컬 본문을 기록하며, provider 쓰기 중 새 입력이 들어오면 방금 기록한 문서를
그 입력의 다음 기준으로 바꾼다. mtime·메타데이터 인덱스 후처리까지 끝나는 동안 들어온 입력도 같은 기준으로 바꾸며,
완료 중인 저장 Job과 교체 예약 Job은 명시적인 Job identity로 구분한다. 기준과 다른 내용이 있으면 외부 변경으로 판정하고
로컬 pending을 쓰지 않는다.

속성 저장은 사전 검사와 pending flush를 나누지 않는다. 하나의 `withWritesPaused` 구간에서 디스크를 읽고 외부 변경 여부,
관리 속성 보호, 최신 로컬 본문 병합, 쓰기와 후속 pending 재기준화를 끝낸다. 외부 본문·frontmatter가 현재 form 기준과
달라졌으면 속성 변경도 저장하지 않는다.

외부 문서를 채택하면 해당 파일의 pending 본문, 속성 초안, Undo/Redo·selection·focus가 속한 `editorSessionKey`를 폐기하고
읽은 디스크 문서를 새 기준으로 게시한다. 이 경우 사용자 선택을 기다리는 문서 충돌 상태를 만들지 않는다.
`documentConflicts`는 provider 파일 이동의 불완전 rollback처럼 디스크 정체성을 자동 판정할 수 없는 무결성 오류에만 유지한다.

앱 내부 rename은 쓰기 fence 안에서 물리 이름 변경이 성공할 때까지 기존 key의 pending을 유지하고, 성공 직후 최신 pending을
새 key로 한 번만 옮긴다. 실패하면 기존 key의 예약을 그대로 둔다. Project 파일 배치 이동은 이동 대상의 입력만 임시 buffer에
보관하고, 재번호되는 다른 파일의 pending은 provider 작업 동안 기존 key에 유지한다. 성공하면 최종 key가 확정된 뒤 최신
pending과 최초 디스크 기준을 함께 옮기고, 완전 rollback이면 복원된 handle에 같은 pending을 다시 연결한다. 이동 중 provider
원문이 바뀌면 그 원문을 우선하고 기존 buffer를 폐기한다. 새 외부 baseline 게시 뒤 시작한 입력은 새 session의 정상 pending으로
남긴다.

### 허용된 트레이드오프

진짜 외부 변경을 반영하면 에디터 블록 ID, 속성 초안, Undo/Redo, focus, cursor, selection이 초기화된다. 외부 우선 정책상
현재는 이를 수용한다. 외부 변경은 앱에서 해당 문서를 편집하지 않던 시점에 발생해야 한다는 사용 전제를 따른다.

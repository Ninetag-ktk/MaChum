...
class FrontMatter private constructor(
    ...
    companion object {
        ...
        fun parse(raw: String): FrontMatter {
            // frontMatter가 없는 경우 _ 시작점 기준
            if (!raw.startsWith("---\n")) {
                return FrontMatter(frontMatterRaw = null, body = raw)
            }
            val end = raw.indexOf("\n---", 4).takeIf { it != -1 }
                ?: return FrontMatter(frontMatterRaw = null, body = raw)
            ...
            return FrontMatter(frontMatterRaw = frontMatterRaw, body = body)
        }
    ...
    fun ensureId(): FrontMatter {
        if (getId() != null) return this
        val newId = generatedId()
        val newLine = "$KEY_ID: $newId"
        // frontMatter가 아예 없으면 새로 생성
        val updateRaw = if (frontMatterRaw == null) {
            newLine
        } else {
            "$newLine\n$frontMatterRaw"
        }
        return FrontMatter(frontMatterRaw = updateRaw, body = body)
    }
    fun inject(): String {

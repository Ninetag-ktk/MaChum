...
| — | CodeBlock 오버레이 (펜스 축소 + lineHeight 패딩 보정) | ✅ 완료 |
...
| — | Callout body fontSize 0.9× / lineHeight 0.9× (sp 절대값, 중첩 누적) | ✅ 완료 |
...
Callout body에서 중첩 Callout/Table/CodeBlock을 별도 오버레이 Composable로 렌더링하는 구조.
...
MarkdownBasicTextField → MarkdownBasicTextFieldCore(depth=0)
  └─ CalloutOverlay (RoundBox + Icon + Title/Body)
       └─ MarkdownBasicTextFieldCore(depth=1)  ← body 재귀 렌더링
            └─ CodeBlockOverlay               ← 코드블록 오버레이
            └─ CalloutOverlay (중첩)
                 └─ MarkdownBasicTextFieldCore(depth=2)
                      └─ (depth >= MAX_OVERLAY_DEPTH(10) → 오버레이 생성 중단, 인라인 서식만)
```
...
- depth=0: `Box + clipToBounds`, depth>0: `OverlayAwareLayout` (overlay offset+height를 부모 측정에 반영)
- `BlockOverlay` → `CalloutOverlay`에 `overlayDepth`, `onRequestActivation`, `contentPadding` 전달
- `CalloutOverlay` RoundBox 디자인 + 타입별 아이콘(18×18)
- `CalloutOverlay` body fontSize × 0.9 + lineHeight × 0.9 (sp 절대값, depth>=1에서만 lineHeight 축소)
- `CalloutOverlay` Title ↔ Body 방향키/Enter 내비게이션 (`FocusRequester`)
- `CodeBlockOverlay` 라운드 배경 + monospace TextField + lineHeight 패딩 보정
- `TableOverlay` lineHeight/2 상하 패딩 (구분자 줄 축소 보정)
- `RawMarkdownOutputTransformation`: `isFocused`, `inlineCodeRanges` 노출
- `BlockDecorationDrawer`: `isNested`, `inlineCodeRanges` 파라미터, Callout RoundBox 테두리, Inline Code RoundRect 배경
- `normalizedTextStyle`: `LineHeightStyle(Proportional, Trim.None)` 적용
- `contentPadding`: fontSize × 3 좌우 패딩 (BasicTextField에만, Dialogue Callout 제외)
- depth=0 `fillMaxSize()`, depth>0 `fillMaxWidth()` (무한 확장 방지)
- 포커스 이탈 시 조건부 raw 동기화 (내용 변경 시에만)
- `onRequestActivation` 콜백: 부모 FocusRequester + 커서 이동 (Callout/Table/CodeBlock 공통)
...
**알려진 이슈 / 주의사항:**
- 오버레이 초기화 1프레임 지연: `textLayoutResult` 확보 전 blockTransparent 적용 → 잠깐 투명 텍스트(오버레이 없음). stale layout 감지 + `lastValidOverlayBlocks`로 완화
- `isFocused` 기본값 `false` 필수: body 생성 시 커서가 텍스트 끝에 위치하므로, `isFocused=true`이면 마지막 줄이 raw로 보임
- depth=0 Callout 높이 overflow: 중첩 overlay의 chrome(padding/icon)이 투명 텍스트보다 커지면 depth=0 Callout이 아래 텍스트를 덮을 수 있음. clipToBounds/fixedHeight 시도했으나 내부 스크롤 충돌로 보류. OverlayAwareLayout이 높이를 측정하므로 depth>0에서는 자동 확장됨
- `forceAllOverlaysInactive` 프로퍼티: 선언은 남아있으나 미사용. `isFocused` 기반 제어로 대체됨. 정리 가능
- depth=0 Callout 위에서 마우스 스크롤 포워딩: body 내부 BasicTextField가 스크롤 이벤트를 소비할 수 있음. 조사 필요
...
| CodeBlock ` ``` ` | 오버레이 (펜스 축소 + monospace + lineHeight 패딩 보정) | ✅ |
